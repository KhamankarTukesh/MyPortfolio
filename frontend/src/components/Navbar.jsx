        ...
        <div className="flex items-center justify-between h-full w-full">
          ...
          <div className='hidden md:flex gap-6'>
            {['Home', 'About', 'Skills', 'Projects', 'Experience', 'Contact'].map((item) => (
              <Link key={item} smooth={true} spy={true} duration={500} activeClass="text-primary" to={item === 'Home' ? '/' : `/${item.toLowerCase()}`} className='text-white hover:text-primary px-3 py-2  rounded-md text-sm font-medium transition-all hover:drop-shadow-[0_0_10px_rgba(57,255,20,0.8)]'>
                {item}
              </Link>
            ))}
          </div>
          ...
          <div className='flex items-center gap-2 sm:gap-4'>
            {/* Resume Button - Ab ye mobile aur desktop dono par dikhega */}
            <button className="py-1.5 px-4 sm:px-6 font-bold rounded-xl border border-primary text-primary bg-background-dark shadow-[0_0_10px_rgba(57,255,20,0.3)] hover:shadow-[0_0_50px_rgba(57,255,20,0.6)] transition-all duration-300 flex items-center hover:scale-[1.05] text-xs sm:text-sm">
              Resume <span className="material-symbols-outlined align-middle ms-2 text-[16px] sm:text-[18px]">download</span>
            </button>
