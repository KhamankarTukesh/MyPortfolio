...
import { motion } from 'framer-motion'; // For Smoothness
...
const Contact = () => {
  ...
  return (
    <motion.div 
      ...
      className="bg-[#0b160a] min-h-screen text-slate-300 font-['Inter'] px-6 flex flex-col items-center selection:bg-[#1fe00e] selection:text-black overflow-x-hidden"
    >
      {/* Header Section */}
      <motion.div variants={itemVariants} className="text-center mb-16 relative mt-24 md:mt-32">
        ...
        <h1 className="text-4xl md:text-5xl font-black text-white mb-4 tracking-tight relative z-10">
          Get In <span className="inline-block drop-shadow-[0_0_10px_rgba(57,255,20,0.8)] animate-pulse text-primary">Touch</span>
        </h1>
        <p className="text-gray-500 text-sm md:text-lg max-w-md mx-auto px-4 leading-relaxed">
      ...
      <div className="max-w-6xl w-full grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-8 items-start mb-20">
        ...
        <motion.div variants={itemVariants} className="order-2 lg:order-1 space-y-12">
          <div className="space-y-4">
            ...
            <motion.div
              ...
              className="group cursor-pointer flex items-center gap-5 p-5 bg-[#132311] border border-white/5 rounded-2xl hover:border-[#1fe00e]/30 transition-all"
            >
            ...
            <div className="flex gap-4">
              {[ {icon: <FaLinkedinIn />, fn: handleLinkedIn}, {icon: <FaGithub />, fn: handleGitHub}, {icon: <SiLeetcode />, fn: handleLeetcode} ].map((item, idx) => (
                <motion.button 
                  ...
                  key={idx} 
                  onClick={item.fn} 
                  className="group relative w-12 h-12 flex items-center justify-center rounded-xl bg-[#132311] border border-white/5 text-gray-400 hover:text-[#38ff14] hover:border-[#38ff14]/50 hover:shadow-[0_0_20px_rgba(56,255,20,0.3)] transition-all duration-300"
                >
                  <div className="absolute inset-0 bg-[#38ff14]/10 rounded-xl blur-lg opacity-0 group-hover:opacity-100 pointer-events-none"></div>
        ...
        <motion.div 
          ...
          className="order-1 lg:order-2 bg-[#132311]/50 border border-white/5 p-6 md:p-8 rounded-[2rem] md:rounded-[2.5rem] relative group overflow-hidden"
        >
          <div className="absolute -inset-10 bg-[#1fe00e]/5 rounded-full blur-[100px] opacity-0 group-hover:opacity-100 transition-opacity duration-1000 pointer-events-none"></div>
          ...
          <form ref={form} onSubmit={sendEmail} className="space-y-5 relative z-10">
            {['user_name', 'user_email', 'subject'].map((field) => (
              <div className="space-y-2" key={field}>
                <label className="text-[10px] uppercase tracking-widest font-black text-gray-500 ml-1">
                  {field.replace('user_', '')} <span className="text-[#1fe00e]">*</span>
                </label>
                <input name={field} type={field === 'user_email' ? 'email' : 'text'} placeholder={`Your ${field.replace('user_', '')}`} className="w-full bg-[#0b160a] border border-white/10 rounded-xl px-5 py-3 text-sm text-white focus:outline-none focus:border-[#1fe00e]/50 transition-all" />
              </div>
            ))}
            ...
            <motion.button 
              ...
              className={`w-full bg-[#1fe00e] hover:bg-[#25ff12] py-4 rounded-xl text-[#0b160a] font-black uppercase tracking-[0.2em] text-xs flex items-center justify-center gap-3 transition-all duration-300 ${status === "sending" ? "opacity-50 cursor-not-allowed" : ""}`}
            >
      ...
      {/* Footer */}
      <motion.footer variants={itemVariants} className="w-full max-w-6xl mt-auto pb-12 pt-8 border-t border-white/5 flex flex-col md:flex-row justify-between items-center text-[9px] text-gray-600 font-bold uppercase tracking-widest gap-4">
